...
#### 6d: Run unit tests with coverage

```
cd <main-repo>/frontend && npx jest --testPathPattern="src/community/<MODULE>" --coverage --coverageReporters=text-summary
```

Capture the test results summary (pass/fail counts) and coverage summary (statements, branches, functions, lines percentages) from the output.
...
#### 6e: Post unit test results as PR comment

After all unit tests pass, post the results and coverage as a comment on the **source PR** (the feature PR in the main monorepo, e.g. `SkappHQ/skapp#<PR_NUMBER>`).

Use `gh` CLI:

```
gh pr comment <PR_NUMBER> --repo SkappHQ/skapp --body "<COMMENT_BODY>"
```

Use this template for the comment body:

```markdown
## 🧪 Unit Test Results — `<MODULE>/<FEATURE>`
...
| Failed | ❌ <FAILED> |
| Skipped | ⏭️ <SKIPPED> |
| Test Suites | <SUITES_PASSED>/<SUITES_TOTAL> passed |
...
| File | Tests | Status |
|------|:-----:|:------:|
| `<test-file-1>.test.ts` | <count> | ✅ |
| `<test-file-2>.test.ts` | <count> | ✅ |
