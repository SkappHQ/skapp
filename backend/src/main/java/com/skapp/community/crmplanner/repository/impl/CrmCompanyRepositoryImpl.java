...
public class CrmCompanyRepositoryImpl implements CrmCompanyRepository {
    ...
    public Page<CrmCompanyMetricsResponseDto> getCompanyMetrics(Pageable pageable, String searchKeyword) {
        ...
        taskSubquery.select(cb.count(subTask.get(CrmTask_.id)))
            .where(cb.equal(subTask.get(CrmTask_.company), company),
                   cb.isFalse(subTask.get(CrmTask_.isDeleted)),
        ...
        openValueSubquery.select(cb.coalesce(cb.sum(openDeal.get(CrmDeal_.amountNumeric)), BigDecimal.ZERO))
            .where(cb.equal(openDeal.get(CrmDeal_.company), company),
                   cb.isFalse(openDeal.get(CrmDeal_.isDeleted)),
        ...
        accountValueSubquery.select(cb.coalesce(cb.sum(closedDeal.get(CrmDeal_.amountNumeric)), BigDecimal.ZERO))
            .where(cb.equal(closedDeal.get(CrmDeal_.company), company),
                   cb.isFalse(closedDeal.get(CrmDeal_.isDeleted)),
        ...
        query.select(
                cb.construct(CrmCompanyMetricsResponseDto.class, 
                        company.get(CrmCompany_.id), 
                        company.get(CrmCompany_.name),
                        company.get(CrmCompany_.contactNumber), 
                        company.get(CrmCompany_.industry),
                        company.get(CrmCompany_.website), 
                        company.get(CrmCompany_.address),
