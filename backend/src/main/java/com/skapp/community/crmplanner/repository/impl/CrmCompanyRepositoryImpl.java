...
public class CrmCompanyRepositoryImpl implements CrmCompanyRepository {
	...
	public Page<CrmCompanyMetricsDto> getCompanyMetrics(Pageable pageable, String searchKeyword) {
		...
		taskSubquery.select(cb.count(subTask.get(CrmConstants.ID)))
			.where(cb.equal(subTask.get(CrmConstants.COMPANY), company),
		...
		overdueSubquery.select(cb.count(subOverdueTask.get(CrmConstants.ID)))
			.where(cb.equal(subOverdueTask.get(CrmConstants.COMPANY), company),
		...
		openValueSubquery.select(cb.coalesce(cb.sum(openDeal.get(CrmConstants.AMOUNT)), BigDecimal.ZERO))
			.where(cb.equal(openDeal.get(CrmConstants.COMPANY), company),
					cb.isFalse(openDeal.get(CrmConstants.IS_DELETED)),
					cb.notEqual(openDeal.get(CrmConstants.STAGE).get(CrmConstants.STAGE_ID), wonStageId));
		...
		accountValueSubquery.select(cb.coalesce(cb.sum(closedDeal.get(CrmConstants.AMOUNT)), BigDecimal.ZERO))
			.where(cb.equal(closedDeal.get(CrmConstants.COMPANY), company),
					cb.isFalse(closedDeal.get(CrmConstants.IS_DELETED)),
					cb.equal(closedDeal.get(CrmConstants.STAGE).get(CrmConstants.STAGE_ID), wonStageId));
		...
		closedCountSubquery.select(cb.count(closedCountDeal.get(CrmConstants.ID)))
			.where(cb.equal(closedCountDeal.get(CrmConstants.COMPANY), company),
					cb.isFalse(closedCountDeal.get(CrmConstants.IS_DELETED)),
					cb.equal(closedCountDeal.get(CrmConstants.STAGE).get(CrmConstants.STAGE_ID), wonStageId));
		...
		openCountSubquery.select(cb.count(openCountDeal.get(CrmConstants.ID)))
			.where(cb.equal(openCountDeal.get(CrmConstants.COMPANY), company),
					cb.isFalse(openCountDeal.get(CrmConstants.IS_DELETED)),
					cb.notEqual(openCountDeal.get(CrmConstants.STAGE).get(CrmConstants.STAGE_ID), wonStageId));

		query.select(
				cb.construct(CrmCompanyMetricsDto.class, company.get(CrmConstants.ID), company.get(CrmConstants.NAME),
	...
	private <T> Predicate[] buildPredicates(CriteriaBuilder cb, From<?, T> root, String searchKeyword) {
		...
		if (StringUtils.hasText(searchKeyword)) {
			...
			predicates.add(cb.like(cb.lower(root.get(CrmConstants.NAME)), likePattern));
		}
	...
	private Long getWonStageId() {
		...
		query.select(stage.get(CrmConstants.STAGE_ID))
			.where(cb.equal(stage.get(CrmConstants.STAGE_TYPE), CrmConstants.WON_STAGE_TYPE),
					cb.isFalse(stage.get(CrmConstants.IS_DELETED)));
