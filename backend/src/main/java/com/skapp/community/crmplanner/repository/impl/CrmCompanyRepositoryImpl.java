...
public class CrmCompanyRepositoryImpl implements CrmCompanyRepository {
	...
	public Page<CrmCompanyTableViewDto> getCompanyTableViewDetails(Pageable pageable, String searchKeyword) {
		...
		Expression<Long> overdueCount = cb.countDistinct(cb.<Long>selectCase()
			.when(cb.and(cb.isFalse(taskJoin.get("isDeleted")), cb.isFalse(taskJoin.get("isCompleted")),
		...
		Expression<BigDecimal> openDealCase = cb.<BigDecimal>selectCase()
			.when(cb.notEqual(dealJoin.get("stage").get("id"), 3), dealJoin.get("amount"))
			.otherwise(BigDecimal.ZERO);

		Expression<BigDecimal> closedDealCase = cb.<BigDecimal>selectCase()
			.when(cb.equal(dealJoin.get("stage").get("id"), 3), dealJoin.get("amount"))
			.otherwise(BigDecimal.ZERO);
		...
		query.select(cb.construct(CrmCompanyTableViewDto.class,

				company.get("id"), company.get("name"), company.get("contactNumber"),
		...
		if (StringUtils.hasText(searchKeyword)) {
			predicates.add(cb.like(cb.lower(company.get("name")), "%" + searchKeyword.toLowerCase() + "%"));
		}
		query.where(predicates.toArray(new Predicate[0]));

		query.groupBy(company.get("id"), company.get("name"), company.get("contactNumber"));
		...
		if (StringUtils.hasText(searchKeyword)) {
			countPredicates.add(cb.like(cb.lower(countRoot.get("name")), "%" + searchKeyword.toLowerCase() + "%"));
		}
		countQuery.select(cb.countDistinct(countRoot.get("id"))).where(countPredicates.toArray(new Predicate[0]));
