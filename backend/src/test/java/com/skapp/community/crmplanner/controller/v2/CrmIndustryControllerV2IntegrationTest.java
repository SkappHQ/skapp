...
class CrmIndustryControllerV2IntegrationTest {
	...
	void createIndustry_AsCrmAdmin_ReturnsCreated() throws Exception {
		performCreateRequest(adminToken, "{\"name\":\"Renewable Energy\"}").andDo(print())
			...
			.andExpect(jsonPath(RESULTS_0_PATH + "['alreadyExists']").value(false))
			.andExpect(jsonPath(RESULTS_0_PATH + "['id']").isNumber());

		assert crmIndustryDao.findByNameIgnoreCaseAndIsDeletedFalse("Renewable Energy").isPresent();
	}
	...
	void createIndustry_AsCrmSalesRepresentative_ReturnsForbidden() throws Exception {
		...
		assert crmIndustryDao.findByNameIgnoreCaseAndIsDeletedFalse("Should Not Be Created").isEmpty();
	}
	...
	@Test
	@DisplayName("Create industry that already exists in a different case - Returns the existing record, no duplicate")
	void createIndustry_DuplicateIgnoringCase_ReturnsExistingRecord() throws Exception {
		...
		performCreateRequest(adminToken, "{\"name\":\"  rETAIL  \"}").andDo(print())
			.andExpect(status().isCreated())
			.andExpect(jsonPath(RESULTS_0_PATH + "['id']").value(existing.getId().intValue()))
			.andExpect(jsonPath(RESULTS_0_PATH + "['name']").value("Retail"))
			.andExpect(jsonPath(RESULTS_0_PATH + "['alreadyExists']").value(true));
	...
	@Test
	@DisplayName("Create industry with collapsible internal whitespace - Matches the existing single-spaced record")
	void createIndustry_InternalWhitespaceCollapsed_MatchesExisting() throws Exception {
		CrmIndustry existing = new CrmIndustry();
