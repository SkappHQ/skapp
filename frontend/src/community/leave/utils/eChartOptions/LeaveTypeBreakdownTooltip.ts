  ...
  const toolTipContent = (): Array<string> => {
    ...
    <div style="display: flex; justify-content: space-between; align-items: center;  padding: 0.3rem 0rem;">
      <div style="display: flex; align-items: center; justify-content: space-between">
        <div style="font-family: ${APP_FONT}; font-style: normal; font-size: 1rem; font-weight: 700; color: #000;line-height: 1rem; margin-right: 0.5rem;">${String(
          item.data
        )}</div>
        ...
        <div style="font-family: ${APP_FONT}; font-style: normal; font-size: 0.75rem; font-weight: 400; line-height: 1rem;">${String(
          item.seriesName
        )}</div>
        </div>
      </div>
