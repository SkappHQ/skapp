...
import { useCancelMyPolicyLeaveRequest } from "~community/leave/api/PolicyLeaveReviewApi";
...
const PolicyCancelLeaveModal: FC<Props> = ({ request, setPopupType }) => {
  ...
  const { mutate: cancelLeaveRequest, isPending } =
    useCancelMyPolicyLeaveRequest(
      () => {
  ...
  const handleCancelLeave = (): void => {
    cancelLeaveRequest({ leaveRequestId: request.leaveRequestId });
  };
