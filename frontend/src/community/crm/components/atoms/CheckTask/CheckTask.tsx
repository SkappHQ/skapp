...
const CheckTask: React.FC<CheckTaskProps> = ({
  ...
  // Internal state gives instant visual feedback without waiting for
  // the parent's controlled prop to round-trip through state updates.
  const [internalChecked, setInternalChecked] = useState(checked);

  // Sync back when the parent changes the prop (e.g. API error revert).
  useEffect(() => {
  ...
  const circleStyles = getCircleStyles(internalChecked, disabled);
  const checkmarkClasses = getCheckmarkClasses(internalChecked, disabled);
