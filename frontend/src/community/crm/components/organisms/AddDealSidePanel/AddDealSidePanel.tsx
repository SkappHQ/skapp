...
// ---------------------------------------------------------------------------
// PropertyRow — shared row layout shell
// ---------------------------------------------------------------------------
...
const AddDealSidePanel: FC = () => {
  ...
  const { data: companies = [] } = useGetCrmCompaniesLookup();

  const [ownerSearchTerm, setOwnerSearchTerm] = useState("");
  const { data: owners = [] } = useGetCrmOwners(ownerSearchTerm);
  ...
  const validationSchema = useMemo(
    () =>
      Yup.object({
        name: Yup.string()
          .trim()
          .required(translateText(["dealNameRequired"])),
  ...
  const selectedOwner = useMemo<CrmOwnerType | null>(
    () =>
      !formik.values.ownerId
        ? null
        : (owners.find(
            (u) => String(u.employeeId) === formik.values.ownerId
          ) ?? null),
  ...
  const selectedContact = useMemo<CrmContactType | null>(
    () =>
      !formik.values.contactId
        ? null
        : (contacts.find((c) => String(c.id) === formik.values.contactId) ??
          null),
    [formik.values.contactId, contacts]
  );

  const selectedCompany = useMemo<CrmCompanyLookupType | null>(
    () =>
      !formik.values.companyId
        ? null
        : (companies.find(
            (co) => String(co.id) === formik.values.companyId
          ) ?? null),
              ...
              <PropertyRow label={translateText(["contactNameLabel"])}>
                <div className="flex flex-col w-full">
                  <ContactPopupSearch
                    ...
                    placeholder={translateText(["noneText"])}
                    searchPlaceholder={translateText(["noneText"])}
                    ariaInvalid={
