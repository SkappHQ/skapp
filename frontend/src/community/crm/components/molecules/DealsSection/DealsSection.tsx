...
import { ContactDeal } from "~community/crm/types/CrmDealTypes";
...
const DealsSection: FC<Props> = ({ contactId }) => {
  ...
  const deals: ContactDeal[] = data?.items ?? [];

  const toDealItem = (deal: ContactDeal): AdvancedAccordionItem => {
    const ownerName =
    ...
    return {
      ...
      statusBadge: deal.stage
        ? {
            text: deal.stage.name,
            iconColor: getStageBadgeColor(deal.stage.name)
          }
        : undefined,
      content: (
