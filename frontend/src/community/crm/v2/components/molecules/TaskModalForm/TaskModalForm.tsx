...
import { getContactFullName } from "~community/crm/v2/utils/crmTaskUtils";
...
const TaskModalForm: FC<TaskFormProps> = ({
  ...
  const selectedTaskId = useCrmStoreV2((state) => state.selectedTaskId);
  const selectedContactId = useCrmStoreV2((state) => state.selectedContactId);
  const selectedCompanyId = useCrmStoreV2((state) => state.selectedCompanyId);
  const isCrmSidePanelOpen = useCrmStoreV2((state) => state.isCrmSidePanelOpen);
  const crmSidePanelType = useCrmStoreV2((state) => state.crmSidePanelType);
  ...
  const [selectedContactName, setSelectedContactName] = useState(
    getContactFullName(panelContact) || getContactFullName(taskContact)
  );
